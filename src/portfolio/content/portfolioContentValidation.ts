...
/**
 * Vérifie si une chaîne est vide une fois les espaces retirés.
 *
 * On s'en sert partout pour repérer un champ visuellement vide, même si le
 * texte contient quelques espaces accidentels.
 */
function isBlank(value: string): boolean {
  return value.trim().length === 0
}
/**
 * Ajoute une erreur standard quand un champ obligatoire manque.
 *
 * Au lieu de répéter le même objet d'erreur partout, cette fonction centralise
 * le format du message et garde les validations plus lisibles.
 */
...
/**
 * Contrôle qu'une image éditoriale contient bien une source et un texte alternatif.
 *
 * Sans `src`, l'image ne peut pas s'afficher. Sans `alt`, elle perd son sens
...
/**
 * Détermine si un lien est cohérent avec son type (`internal`, `static` ou `external`).
 *
 * Cette étape évite de publier un lien interne cassé ou une URL externe mal formée.
 */
...
/**
 * Transforme un lien invalide en erreur compréhensible pour l'éditeur de contenu.
 *
 * Le but est de dire clairement ce qui ne va pas, sans demander au rédacteur de
 * relire la structure TypeScript.
 */
...
/**
 * Parcourt les sections d'une entrée et vérifie qu'elles sont complètes.
 *
 * Cette fonction est importante parce qu'une entrée peut contenir plusieurs
 * blocs différents: texte, image, citation, métadonnées ou lien.
 */
...
/**
 * Vérifie les champs communs à tous les contenus publiés.
 *
 * Ici on contrôle les éléments de base: slug, titre, statut, résumé, année et
 * sections. Si cette base est saine, les pages peuvent se construire ensuite.
 */
...
/**
 * Ajoute les règles spécifiques à un projet du portfolio.
 *
 * Un projet publié doit avoir une introduction, une image de couverture et des
 * liens cohérents.
 */
...
/**
 * Ajoute les règles spécifiques à une scène sonore.
 *
 * Une scène peut rester brouillon, mais si elle est publiée elle doit garder une
 * image d'illustration correcte.
 */
...
/**
 * Ajoute les règles spécifiques à une entrée de laboratoire.
 *
 * Le lab demande un texte d'introduction, un type d'expérience et une image de
 * couverture pour rester lisible dans l'index.
 */
...
/**
 * Cette classe structure le sujet “portfolio Content Error” dans portfolio.
 *
 * Fichier: src/portfolio/content/validation.ts
 * Si tu lis ce fichier pour apprendre, regarde d’abord PortfolioContentError dans validation.ts.
 */
