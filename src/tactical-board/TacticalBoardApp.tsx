...
import { useBoardAssetUrls } from './hooks/useBoardAssetUrls'
import { useBoardOnlyMode } from './hooks/useBoardOnlyMode'
import { useGlobalBoardShortcuts } from './hooks/useGlobalBoardShortcuts'
import { useScenarioAutosave } from './hooks/useScenarioAutosave'
import { createImageAsset } from './import-export/imageAssets'
import {
  exportAllScenariosJson,
  exportScenarioJson,
  importScenario,
} from './import-export/scenarioExchange'
import { exportBoardToPng } from './import-export/exportBoardImage'
import { previewResize } from './model/applyScenarioCommand'
import { deriveSelectionModel } from './model/boardSelection'
import { createNextScenario } from './model/scenarioContinuity'
import { createDefaultScenario, createEntityId } from './model/scenarioDocument'
import { sortScenarios, uniqueScenarioName } from './model/scenarioNaming'
import type {
  AnnotationChanges,
  CustomUnitType,
  Faction,
  ...
  UnitStatus,
  UnitType,
} from './model/tacticalBoardTypes'
import { BUILT_IN_UNIT_TYPES } from './model/unitCatalog'
import { loadInitialBoardData } from './persistence/loadInitialBoardData'
import { tacticalBoardRepository } from './persistence/tacticalBoardRepository'
import { selectActiveScenario, useTacticalBoardStore } from './state/tacticalBoardStore'
import { Button } from './ui/Button'
import type { SaveState as UiSaveState } from './ui/SaveStatus'
import { Battlefield } from './features/battlefield/Battlefield'
import { BattlefieldSettingsModal } from './features/battlefield/BattlefieldSettingsModal'
import { BattlefieldToolbar } from './features/battlefield/BattlefieldToolbar'
...
} from './features/scenarios/NewScenarioModal'
import { ScenarioTopBar } from './features/scenarios/ScenarioTopBar'
import {
  NextScenarioModal,
...
export default function TacticalBoardApp() {
  ...
  const documents = useTacticalBoardStore((state) => state.documents)
  const history = useTacticalBoardStore((state) => state.history)
  const hydrated = useTacticalBoardStore((state) => state.hydrated)
  const tool = useTacticalBoardStore((state) => state.tool)
  const selectedTypeId = useTacticalBoardStore((state) => state.selectedTypeId)
  const activeFactionId = useTacticalBoardStore((state) => state.activeFactionId)
  const selection = useTacticalBoardStore((state) => state.selection)
  const zoom = useTacticalBoardStore((state) => state.zoom)
  const arrowStyle = useTacticalBoardStore((state) => state.arrowStyle)
  const arrowColor = useTacticalBoardStore((state) => state.arrowColor)
  const markerKind = useTacticalBoardStore((state) => state.markerKind)
  const markerColor = useTacticalBoardStore((state) => state.markerColor)
  const leftPanelOpen = useTacticalBoardStore((state) => state.leftPanelOpen)
  const rightPanelOpen = useTacticalBoardStore((state) => state.rightPanelOpen)
  const notification = useTacticalBoardStore((state) => state.notification)

  const hydrate = useTacticalBoardStore((state) => state.hydrate)
  const commit = useTacticalBoardStore((state) => state.commit)
  const undo = useTacticalBoardStore((state) => state.undo)
  const redo = useTacticalBoardStore((state) => state.redo)
  const setActiveScenario = useTacticalBoardStore((state) => state.setActiveScenario)
  const addDocument = useTacticalBoardStore((state) => state.addDocument)
  const removeDocument = useTacticalBoardStore((state) => state.removeDocument)
  const setTool = useTacticalBoardStore((state) => state.setTool)
  const setSelectedTypeId = useTacticalBoardStore((state) => state.setSelectedTypeId)
  const setActiveFactionId = useTacticalBoardStore((state) => state.setActiveFactionId)
  const setSelection = useTacticalBoardStore((state) => state.setSelection)
  const setZoom = useTacticalBoardStore((state) => state.setZoom)
  const setArrowStyle = useTacticalBoardStore((state) => state.setArrowStyle)
  const setArrowColor = useTacticalBoardStore((state) => state.setArrowColor)
  const setMarkerKind = useTacticalBoardStore((state) => state.setMarkerKind)
  const setMarkerColor = useTacticalBoardStore((state) => state.setMarkerColor)
  const setLeftPanelOpen = useTacticalBoardStore((state) => state.setLeftPanelOpen)
  const setRightPanelOpen = useTacticalBoardStore((state) => state.setRightPanelOpen)
  const notify = useTacticalBoardStore((state) => state.notify)
  const clearNotification = useTacticalBoardStore((state) => state.clearNotification)
  ...
  const { assetUrls, loadAssets, refreshAssetUrls } = useBoardAssetUrls()
  const {
    saveSnapshot,
  ...
  const finishMultiSelection = useCallback(() => {
    const currentSelection = useTacticalBoardStore.getState().selection
