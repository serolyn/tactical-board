...
export interface DefaultScenarioOptions {
  ...
  readonly now?: string
  /** @deprecated Prefer ownForcesFactionId. */
  readonly blueFactionId?: EntityId
  /** @deprecated Prefer obstacleFactionId. */
...
function inferLegacyFactionRole(name: string, index: number): FactionRole {
  ...
  // V1 created Bleu then Rouge. The index fallback also migrates renamed default factions.
...
/**
 * Pure V1 → V2 migration. Existing board data and entity identifiers are retained verbatim;
 * only semantic faction roles and the new scenario defaults are added.
 */
